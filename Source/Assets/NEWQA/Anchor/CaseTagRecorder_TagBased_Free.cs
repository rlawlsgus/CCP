...
public class CaseTagRecorder_NoAnchors_Free : MonoBehaviour
{
    ...
    [Tooltip("⚠️ 이제 Trigger 여부로 필터링하지 않음(요청대로 trigger on/off 상관없이). 필드만 남겨둠.")]
    ...
    // ===================== SphereCast Debug/Tag Settings =====================
    [Header("Env Distance = SphereCast (XZ)")]
    [Tooltip("SphereCast 시작점 높이(허리/가슴). 위아래 길쭉한 객체에서 옆면 기준 거리로 안정화됨")]
    public float sphereCastOriginYOffset = 0.9f;

    [Tooltip("SphereCast probe 반경(센서 두께). 0.2~0.4 추천")]
    public float sphereCastProbeRadius = 0.25f;

    [Tooltip("SphereCast miss 시 fallback으로 HorizontalDistance(ClosestPoint+XZ) 사용")]
    ...
    // ✅ SphereCast hits buffer
    private readonly RaycastHit[] _sphereHitBuf = new RaycastHit[256];
    ...
    void EnsureAgentMainCollider(AgentRuntime ar)
    {
        if (ar == null || ar.tr == null) return;

        var go = ar.tr.gameObject;
        if (go == null) return;

        var col = go.GetComponentInChildren<Collider>(true);
        if (col == null)
        {
            // 없으면 최소 콜라이더 추가(ComputePenetration/HIT용)
    ...
    void RecordFrame(int gf)
    {
        foreach (var kv in _runtimeById)
        {
            ...
            if (ar.mainCollider == null) EnsureAgentMainCollider(ar);
            ...
            // ✅ 여기만 SphereCast/penetration 기반으로 교체
            ComputeTaggedDistances(ar, pos,
            ...
            if (hasMeta)
            {
                ...
                {
                    tag.groupCenterWorld = centerWorld;
                    tag.groupCenterRelWorld = centerWorld - pos;
    ...
    // ----------------- Proximity/Hit (SphereCast 기반) -----------------
    bool ShouldConsiderCollider(Collider c)
    {
        if (c == null) return false;
        // ✅ trigger on/off 상관없이: 더 이상 isTrigger로 필터링하지 않음
        return true;
    }
    ...
    )
    {
        ...
        if (self == null || self.tr == null) { nearestObs = nearestBld = nearestEnt = nearestVeh = -1f; return; }
        if (self.mainCollider == null) EnsureAgentMainCollider(self);

        // ✅ trigger 포함 고정
        QueryTriggerInteraction qti = QueryTriggerInteraction.Collide;

        // ✅ 후보 수집은 maxNear + probeRadius 정도로(캐스트 miss 줄이기)
        float gatherR = _envMaxNearRadius + sphereCastProbeRadius;
        int nNear = Physics.OverlapSphereNonAlloc(pos, gatherR, _nearBuf, ~0, qti);

        Vector3 castOrigin = pos + Vector3.up * sphereCastOriginYOffset;
        ...
        for (int i = 0; i < nNear; i++)
        {
            ...
            // 자기 자신 콜라이더면 스킵
            if (c.transform == self.tr || c.transform.IsChildOf(self.tr)) continue;

            var kind = GetEnvKind(c);
            if (kind == EnvKind.None) continue;

            // ✅ HIT(진짜 겹침) 우선
            bool overlapping = (self.mainCollider != null) && IsOverlapping(self.mainCollider, c);
            ...
            if (!overlapping)
            {
                ...
                {
                    d = HorizontalDistanceToCollider(pos, c);
                    ok = true;
                }
            ...
            switch (kind)
            {
                case EnvKind.Obstacle:
                    ...
                    if (overlapping || d <= hitRadiusObstacle) hitObs = true;
                    break;

                case EnvKind.Building:
                    if (d < nearestBld) nearestBld = d;
                    if (overlapping || d <= hitRadiusBuilding) hitBld = true;
                    break;

                case EnvKind.Entarance:
                    if (d < nearestEnt) nearestEnt = d;
                    if (overlapping || d <= hitRadiusEntarance) hitEnt = true;
                    break;

                case EnvKind.Vehicle:
                    if (d < nearestVeh) nearestVeh = d;
                    if (overlapping || d <= hitRadiusVehicle) hitVeh = true;
                    break;
            }
        }
        ...
        out float bestDistance)
    {
        ...
        int hitCount = Physics.SphereCastNonAlloc(origin, radius, dir, _sphereHitBuf, maxDistance, ~0, qti);
        bool found = false;
