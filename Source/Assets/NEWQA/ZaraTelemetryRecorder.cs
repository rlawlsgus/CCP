...
public class ZaraTelemetryRecorder : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        foreach (var kv in tracks)
        {
            ...
            if (!active)
            {
                // inactive: pos는 현재 Transform이 유지되는 경우도 있으니 그냥 기록은 해두되,
                // group padding은 lastObservedWorld를 사용하도록 설계함.
    ...
    {
        // 1) active든 아니든: "관측된 적"이 있으면 lastObservedWorld 사용 (forward-fill)
        if (tracks.TryGetValue(memberId, out var mt) && mt != null)
        {
            if (mt.everObserved) return mt.lastObservedWorld;
        }

        // 2) 관측 전이면: trajectory 시작점 사용 (back-fill에 해당)
