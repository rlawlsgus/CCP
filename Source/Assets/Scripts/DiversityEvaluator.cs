...
public class DiversityEvaluator : MonoBehaviour
{
    ...
    [Tooltip("Maximum measurement duration in seconds. 0 means no limit.")]
    ...
    void Update()
    {
        ...
        if (maxDuration > 0)
        {
            ...
            if (elapsedTimer >= maxDuration)
            {
                Debug.Log($"[DiversityEvaluator] Time limit of {maxDuration}s reached. Stopping recording and saving.");
                isRecording = false;
                SavePathsToFile();
                return;
            }
    ...
    void FindAgents()
    {
        // Find all Agent_Training and Agent_GoalOnly_Training components in the scene
        var trainingAgents = FindObjectsOfType<Agent_Training>();
        var goalOnlyAgents = FindObjectsOfType<Agent_GoalOnly_Training>();

        // Combine unique transforms
        HashSet<Transform> foundTransforms = new HashSet<Transform>();
        foreach(var agent in trainingAgents) foundTransforms.Add(agent.transform);
        foreach(var agent in goalOnlyAgents) foundTransforms.Add(agent.transform);

        foreach (var agentTransform in foundTransforms)
        {
            if (trackedAgents.ContainsKey(agentTransform)) continue;

            int id = ParseAgentID(agentTransform.gameObject.name);
            
            // Check if ID is within range
            if (id != -1 && id >= minAgentID && id <= maxAgentID)
            {
                trackedAgents.Add(agentTransform, id);
                if (!agentPaths.ContainsKey(id))
                {
                    agentPaths[id] = new List<Vector3>();
                }
                // Debug.Log($"[DiversityEvaluator] Started tracking Agent {id} ({agentTransform.name})");
            }
        }
    }
    ...
    {
        if (agentPaths.Count == 0 || isSaved) return;
