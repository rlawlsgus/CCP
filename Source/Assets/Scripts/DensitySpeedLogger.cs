...
public class DensitySpeedLogger : MonoBehaviour
{
    ...
    public enum SamplingMode { TimeBased, FrameBased }

    [Header("Sampling Settings")]
    public SamplingMode samplingMode = SamplingMode.TimeBased;

    [Tooltip("Time interval between measurements (seconds). Higher values smooth out jitter.")]
    public float sampleInterval = 0.5f;

    [Tooltip("Number of frames between measurements. Only used if SamplingMode is FrameBased.")]
    ...
    void Update()
    {
        ...
        if (samplingMode == SamplingMode.TimeBased)
        {
    ...
    bool CheckGoalReached(GameObject agent)
    {
        ...
        if (!reached)
        {
            var trainingAgent = agent.GetComponent<Agent_Training>();
            if (trainingAgent != null && trainingAgent.reachedGoal) reached = true;
        }
        if (!reached)
        {
            var goalOnlyAgent = agent.GetComponent<Agent_GoalOnly_Training>();
            if (goalOnlyAgent != null && goalOnlyAgent.reachedGoal) reached = true;
        }
    ...
    void OnGUI()
    {
        ...
        GUILayout.Label($"Mode: {samplingMode}");
        if (samplingMode == SamplingMode.TimeBased)
            GUILayout.Label($"Interval: {sampleInterval}s");
        else
            GUILayout.Label($"Interval: {sampleFrames} frames");
