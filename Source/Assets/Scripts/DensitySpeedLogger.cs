...
public class DensitySpeedLogger : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (prevPositions.TryGetValue(id, out Vector3 prevPosFlat))
            {
                ...
                // Filter out teleportations or unrealistic speeds
    ...
    bool CheckGoalReached(GameObject agent)
    {
        ...
        // Check for Agent_Training reachedGoal
        if (!reached) {
            var training = agent.GetComponent<Agent_Training>();
            if (training != null && training.reachedGoal) reached = true;
        }

        // Check for Agent_GoalOnly_Training reachedGoal
        if (!reached) {
            var goalOnly = agent.GetComponent<Agent_GoalOnly_Training>();
            if (goalOnly != null && goalOnly.reachedGoal) reached = true;
        }
